...
const UserProfilePage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f7f2e7]">
      {/* Hero Section */}
      <div className="bg-gradient-to-br from-[#2f4823] to-[#1f3219] text-white py-16 md:py-20 px-4 text-center">
      ...
      <div className="container mx-auto px-3 sm:px-4 py-8 sm:py-12 max-w-6xl">
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg border border-[#779385]/20 overflow-hidden">

          {/* Header with User Info - MEJORADO RESPONSIVE */}
          <div className="bg-gradient-to-r from-[#f7f2e7] to-[#e8dfca] p-4 sm:p-6 border-b border-[#779385]/20">
            <div className="flex flex-col lg:flex-row items-center justify-between space-y-4 lg:space-y-0">
              ...
              <div className="flex items-center space-x-3 sm:space-x-4">
                <div className="relative">
                  ...
                  />
                  {!user.picture && (
                    <div className="w-12 h-12 sm:w-16 sm:h-16 bg-[#2f4823] text-white rounded-full flex items-center justify-center font-semibold text-lg sm:text-xl border-4 border-white shadow-lg">
                      {user.name?.charAt(0)?.toUpperCase() || 'U'}
                    </div>
                  )}
                </div>
                <div className="text-center sm:text-left">
                  <h2 className="text-xl sm:text-2xl font-bold text-[#2f4823] font-serif">
                    {user.name}
                  </h2>
                  <p className="text-[#779385] text-sm sm:text-base">{user.email}</p>
              ...
              {/* Buttons - MEJORADO PARA MÓVIL */}
              <div className="flex flex-col sm:flex-row items-center space-y-2 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
                <button
                  onClick={() => navigate('/shop-page')}
                  className="w-full sm:w-auto px-4 py-2 border border-[#2f4823] text-[#2f4823] rounded-xl sm:rounded-2xl font-semibold hover:bg-[#2f4823] hover:text-white transition-all duration-300 text-sm sm:text-base"
                >
                  Seguir Comprando
                </button>
                <button
                  ...
                  className="w-full sm:w-auto px-4 py-2 bg-red-500 text-white rounded-xl sm:rounded-2xl font-semibold hover:bg-red-600 transition-all duration-300 text-sm sm:text-base"
                >
                  Cerrar Sesión
                </button>
              </div>
          ...
          {/* Navigation Tabs - MEJORADO RESPONSIVE */}
          {/* Navigation Tabs - VERSIÓN GRID PARA MÓVIL */}
            ...
            <div className="grid grid-cols-3 sm:hidden">
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex flex-col items-center px-2 py-3 font-semibold transition-all duration-300 ${activeTab === tab.id
            ...
            <div className="hidden sm:flex">
              {tabs.map((tab) => (
                <button
                  key={tab.id}
                  onClick={() => setActiveTab(tab.id)}
                  className={`flex items-center space-x-2 px-6 py-4 font-semibold transition-all duration-300 whitespace-nowrap ${activeTab === tab.id
                      ? 'text-[#2f4823] border-b-2 border-[#2f4823] bg-[#f7f2e7]'
                      : 'text-[#779385] hover:text-[#2f4823] hover:bg-[#f7f2e7]/50'
                    }`}
                >
